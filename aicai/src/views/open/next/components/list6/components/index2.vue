      ...
      <van-cell v-for="(item,l) in list" :key="l">
       <div class="nxt">
         <div>
           <p>{{item.expect}}期</p>
           <div>
             <div v-for="(it,k) in item.opencode.split(',').slice(0,item.opencode.split(',').length-1)" :key="k">
               <i  :style="{background:ball(it)}" >{{it}}</i>
               <p>{{sxSort(it)}}</p>
             </div>
             <van-icon color="#333333" style="background: none" name="plus" />
             <div>
               <i  :style="{background:ball(item.opencode.split(',').slice(item.opencode.split(',').length-1))}" >{{parseInt(item.opencode.split(',').slice(item.opencode.split(',').length-1))}}</i>
               <p>{{sxSort(item.opencode.split(',').slice(item.opencode.split(',').length-1)[0])}}</p>
             </div>
           </div>
         </div>
