<template>
  <div class="red" v-if="list">
    <div><p v-for="(item,k) in list" :key="k">{{ item.expect }}期</p></div>
    <div>
      <div v-for="(item,k) in list" :key="k">
        <a v-for="ks in 35" :key="ks">
          <span :class="{active:item.opencode.split(',').slice(0,5).indexOf(ks.toString().length === 1 ? '0' + ks : ks.toString())!==-1}">
